...
$\mathsf{snd}(x)$, respectively. Additionally, $A_1 \oplus \cdots \oplus A_n$ denotes the
disjoint union of $A_1, \dots, A_n$, so $\mathsf{inj_i}(a_i)$ are the
corresponding injections into $A_1 \oplus \cdots \oplus A_n$ from $A_i$ for all $i \in
\{1, \dots, n\}$. Given a type $A$, we can define the optional type $\Option{A}$
as $A \oplus \bot$, where $\bot$ is the bottom type.
...
\mathsf{Effect}(S_j) ::= &\quad \mathsf{send}(\mathsf{Addr} \oplus \mathsf{Name}, \mathsf{Msg}) & &\text{(message dispatch)} \\
...
\begin{definition}\label{def:guard-action}
  ...
  $\mathsf{Msg} \times  \mathsf{Cfg}_j \times \mathsf{Env}_j(S_j) \to \mathsf{Effect}(S_j)$,
