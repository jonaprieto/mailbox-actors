...
$\Option{A}$ as $A \oplus \bot$, where $\bot$ is the bottom type.
...
returns nothing if the guard is not satisfied, and returns data otherwise.
...
this, we need to show that our design satisfies the two fundamental requirements:
conditional branching through guard-state interactions and indefinite looping
via message-passing recursion. However, note that the message-passing mechanism
...
A \emph{mailbox engine} $\mathsf{m}_j$ is a dedicated processing engine 
created by the system automatically just before a processing engine $e_j$ is created
...
$\mathsf{Msg_{m_j}} := \mathsf{append}(\mathsf{Msg_j}) \cup
