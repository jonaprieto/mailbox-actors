...
In the system, not all engines are processing engines. Some engines are specialized
in handling messages, and are referred to as \emph{mailbox engines} and treated
differently by the system's message passing mechanism.

\begin{definition}\label{def:mailbox-engine} A \emph{mailbox engine}, denoted as
$\mathsf{m}_j$ by some address $j$, is a distinguished and at the same time
opage engine within the system. It is created by the system automatically when
a non-mailbox processing engine $e_j$ is created with the following specific properties:
\begin{itemize}
\item the parent of the mailbox engine is the processing engine $e_j$,
\item the message interface of the mailbox engine is fixed and only supports two message types: 
$\mathsf{append}$ and $\mathsf{pull}$.
$\mathsf{Msg_{m_j}} := \mathsf{append}(\mathsf{Msg_j}) \mid \mathsf{pull}(\mathsf{Msg_j})$.
\item the state of the mailbox engine is implemented as an abstract message
store with implementation-defined ordering semantics
\item the mailbox engine is created automatically with the parent processing engine
\end{itemize}
